...
package com.pramid.ml.cv.boofcv.tracking;
...
/**
 * Example of how to use the low level implementation of mean-shift to track a
 * specific color provided by the user. The weights of each pixel is computed by
 * RgbLikelihood, see below, and track regions are rectangular. This tracker
 * works very well since the ball is almost uniformly blue. If the light varried
 * then a HSV color model might be better.
 *
 * @author Peter Abeles
 */
public class ExampleTrackerMeanShiftLikelihood {
	...
	/**
	 * Very simple implementation of PixelLikelihood. Uses linear distance to
	 * compute how close a color is to the target color.
	 */
	public static class RgbLikelihood implements
		...
		/**
		 * This function is used to learn the target's model from the select
		 * image region. Since the model is provided in the constructor it isn't
		 * needed or used.
		 */
		@Override
		...
		public float compute(int x, int y) {
			...
			float red = Math.max(0, 1.0f - Math.abs(targetRed - pixelR)
					/ radius);
			float green = Math.max(0, 1.0f - Math.abs(targetGreen - pixelG)
					/ radius);
			float blue = Math.max(0, 1.0f - Math.abs(targetBlue - pixelB)
					/ radius);
	...
	public static void main(String[] args) {
		...
		RectangleLength2D_I32 location = new RectangleLength2D_I32(394, 247,
				475 - 394, 325 - 247);

		ImageType<Planar<GrayU8>> imageType = ImageType.pl(3, GrayU8.class);

		SimpleImageSequence<Planar<GrayU8>> video = media.openVideo(fileName,
				imageType);

		// Return a higher likelihood for pixels close to this RGB color
		RgbLikelihood likelihood = new RgbLikelihood(64, 71, 69);

		TrackerMeanShiftLikelihood<Planar<GrayU8>> tracker = new TrackerMeanShiftLikelihood<Planar<GrayU8>>(
				likelihood, 50, 0.1f);

		// specify the target's initial location and initialize with the first
		// frame
		Planar<GrayU8> frame = video.next();
		// Note that the tracker will not automatically invoke
		// RgbLikelihood.createModel() in its initialize function
