...
/**
 * <p>
 * Example of how to use the {@link boofcv.abst.feature.tracker.PointTracker} to
 * track different types of point features. ImagePointTracker hides much of the
 * complexity involved in tracking point features and masks the very different
 * underlying structures used by these different trackers. The default trackers
 * provided in BoofCV are general purpose trackers, that might not be the best
...
public class ExamplePointFeatureTracker<T extends ImageGray, D extends ImageGray> {
	...
	/**
	 * Processes the sequence of images and displays the tracked features in a
	 * window
	 */
	...
	private void updateGUI(SimpleImageSequence<T> sequence) {
		...
		// draw tracks with semi-unique colors so you can track individual
		// points with your eyes
		for (PointTrack p : tracker.getActiveTracks(null)) {
			...
			VisualizeFeatures.drawPoint(g2, (int) p.x, (int) p.y, new Color(
					red, green, blue));
		}
	...
	public void createKLT() {
		...
		tracker = FactoryPointTracker.klt(config, new ConfigGeneralDetector(
				600, 6, 1), imageType, derivType);
	}
	...
	public void createSURF() {
		...
		tracker = FactoryPointTracker.dda_FH_SURF_Fast(configDetector, null,
				null, imageType);
	}
	...
	public static void main(String args[]) throws FileNotFoundException {
		...
		int pause;
		SimpleImageSequence sequence = media.openVideo(
				UtilIO.pathExample("zoom.mjpeg"), ImageType.single(imageType));
		pause = 100;
		// media.openCamera(null,640,480,ImageType.single(imageType)); pause =
		// 5;
		sequence.setLoop(true);

		ExamplePointFeatureTracker app = new ExamplePointFeatureTracker(
				imageType, pause);
