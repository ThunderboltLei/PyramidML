...
package com.pramid.ml.cv.boofcv.imageprocessing;
...
public class ExampleFourierTransform {
	 ...
	 * compares the results to a box filter which has been applied in the
	 * spatial domain
	 */
	public static void applyBoxFilter(GrayF32 input) {
		...
		InterleavedF32 boxTransform = new InterleavedF32(input.width,
				input.height, 2);
		InterleavedF32 transform = new InterleavedF32(input.width,
				input.height, 2);
		GrayF32 blurredImage = new GrayF32(input.width, input.height);
		GrayF32 spatialBlur = new GrayF32(input.width, input.height);

		DiscreteFourierTransform<GrayF32, InterleavedF32> dft = DiscreteFourierTransformOps
				.createTransformF32();
		...
		// create the box filter which is centered around the pixel. Note that
		// the filter gets wrapped around
		// the image edges
		for (int y = 0; y < 15; y++) {
		...
		// Visualize the Fourier Transform for the input image and the box
		// filter
		displayTransform(transform, "Input Image");
		displayTransform(boxTransform, "Box Filter");

		// apply the filter. convolution in spacial domain is the same as
		// multiplication in the frequency domain
		DiscreteFourierTransformOps.multiplyComplex(transform, boxTransform,
				transform);
		...
		// For sake of comparison, let's compute the box blur filter in the
		// spatial domain
		// NOTE: The image border will be different since the frequency domain
		// wraps around and this implementation
		...
		BufferedImage spacialOut = ConvertBufferedImage.convertTo(spatialBlur,
				null);
		BufferedImage blurredOut = ConvertBufferedImage.convertTo(blurredImage,
				null);
		...
		ShowImages.showWindow(listPanel,
				"Box Blur in Spacial and Frequency Domain of Input Image");
	}
	...
	public static void displayTransform(InterleavedF32 transform, String name) {
		...
		// shift the zero-frequency into the image center, as is standard in
		// image processing
		...
		// Display the results
		BufferedImage visualMag = VisualizeImageData.grayMagnitude(magnitude,
				null, -1);
		BufferedImage visualPhase = VisualizeImageData.colorizeSign(phase,
				null, Math.PI);
	...
	public static void main(String args[]) {
		...
		GrayF32 input = UtilImageIO.loadImage(
				UtilIO.pathExample("standard/lena512.jpg"), GrayF32.class);
