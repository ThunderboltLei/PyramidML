...
/**
 * Example that demonstrates how to detect calibration targets. Calibration
 * points are found on the targets to a high level of precision. It is assumed
 * that a single image only shows a single target and that the entire target is
 * visible. If these conditions are not meet then the target is likely to not be
 * detected.
 *
 * @author Peter Abeles
 */
public class ExampleDetectCalibrationPoints {
	...
	public static void main(String args[]) {
		...
		// load the test image
		// String directory =
		// UtilIO.pathExample("calibration/stereo/Bumblebee2_Square");
		...
		// detector = FactoryCalibrationTarget.detectorSquareGrid(new
		// ConfigSquareGrid(4, 3, 30, 30));
