...
/**
 * Demonstration of how to convert a point sequence describing an objects
 * outline/contour into a sequence of line segments. Useful when analysing
 * shapes such as squares and triangles or when trying to simply the low level
 * pixel output.
 *
 * @author Peter Abeles
 */
public class ExampleFitPolygon {
	...
	public static void fitBinaryImage(GrayF32 input) {
		...
		for (Contour c : contours) {
			...
			for (List<Point2D_I32> internal : c.internal) {
				vertexes = ShapeFittingOps.fitPolygon(internal, true,
	...
	public static void fitCannyEdges(GrayF32 input) {
		...
		BufferedImage displayImage = new BufferedImage(input.width,
				input.height, BufferedImage.TYPE_INT_RGB);
		...
		for (EdgeContour e : contours) {
			...
			for (EdgeSegment s : e.segments) {
				...
				// false
				List<PointIndex_I32> vertexes = ShapeFittingOps.fitPolygon(
						s.points, false, splitFraction, minimumSideFraction,
						100);
	...
	public static void fitCannyBinary(GrayF32 input) {
		...
		// Finds edges inside the image
		CannyEdge<GrayF32, GrayF32> canny = FactoryEdgeDetectors.canny(2,
				false, true, GrayF32.class, GrayF32.class);
	...
	public static void main(String args[]) {
		...
		BufferedImage image = UtilImageIO.loadImage(UtilIO
				.pathExample("shapes/shapes02.png"));
