...
package com.pramid.ml.cv.boofcv.segmentation;
...
public class ExampleSegmentSuperpixels {
	 ...
	 */
	public static <T extends ImageBase> void performSegmentation(
			ImageSuperpixels<T> alg, T color) {
		// Segmentation often works better after blurring the image. Reduces
		// high frequency image components which
		// can cause over segmentation
		GBlurImageOps.gaussian(color, color, 0.5, -1, null);

		// Storage for segmented image. Each pixel will be assigned a label from
	...
	public static <T extends ImageBase> void visualize(GrayS32 pixelToRegion,
			T color, int numSegments) {
		// Computes the mean color inside each region
		ImageType<T> type = color.getImageType();
		ComputeRegionMeanColor<T> colorize = FactorySegmentationAlg
				.regionMeanColor(type);
		...
		ImageSegmentationOps.countRegionPixels(pixelToRegion, numSegments,
				regionMemberCount.data);
		colorize.process(color, pixelToRegion, regionMemberCount, segmentColor);

		// Draw each region using their average color
		BufferedImage outColor = VisualizeRegions.regionsColor(pixelToRegion,
				segmentColor, null);
		// Draw each region by assigning it a random color
		BufferedImage outSegments = VisualizeRegions.regions(pixelToRegion,
				numSegments, null);
	...
	public static void main(String[] args) {
		BufferedImage image = UtilImageIO.loadImage(UtilIO
				.pathExample("segment/berkeley_horses.jpg"));
		// BufferedImage image =
		// UtilImageIO.loadImage(UtilIO.pathExample("segment/berkeley_kangaroo.jpg"));
		// BufferedImage image =
		// UtilImageIO.loadImage(UtilIO.pathExample("segment/berkeley_man.jpg"));
		// BufferedImage image =
		// UtilImageIO.loadImage(UtilIO.pathExample("segment/mountain_pines_people.jpg"));
		// BufferedImage image =
		// UtilImageIO.loadImage(UtilIO.pathExample("particles01.jpg"));

		// you probably don't want to segment along the image's alpha channel
		// and the code below assumes 3 channels
		image = ConvertBufferedImage.stripAlphaChannel(image);

		// Select input image type. Some algorithms behave different depending
		// on image type
		...
		// ImageSuperpixels alg = FactoryImageSegmentation.meanShift(null,
		// imageType);
		// ImageSuperpixels alg = FactoryImageSegmentation.slic(new
		// ConfigSlic(400), imageType);
		ImageSuperpixels alg = FactoryImageSegmentation.fh04(new ConfigFh04(
				100, 30), imageType);
		// ImageSuperpixels alg =
		// FactoryImageSegmentation.watershed(null,imageType);

		// Convert image into BoofCV format
		ImageBase color = imageType.createImage(image.getWidth(),
				image.getHeight());
